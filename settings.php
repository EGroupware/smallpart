				...
				if ($user['Superadmin']) {
					...
					<div role="tabpanel" class="tab-pane" id="Userpasswort">
						...
						<form action="?save=Userpasswort" method="post" class="form-horizontal">
							<div class="form-group">
								<select name="UserToCanchePWID" id="UserToCanchePWID"
								        style="font-size: x-large; min-width: 300px;">
									<?php
										$stmt1 = $pdo->prepare("SELECT * FROM users ORDER BY email");
										$stmt1->execute();
										...
										foreach ($results as $result) {
											...
											echo '<option value="' . $result["id"] . '"  >' . $i . ".&nbsp;&nbsp;&nbsp;&nbsp;" . $result["email"] . "&nbsp;&nbsp;&nbsp; / &nbsp;&nbsp;&nbsp;" . $result["nickname"] . "&nbsp;&nbsp;&nbsp; ([ID: " . $result["id"] . ' | Rolle: ' . $rolen . ' ])</option>';
										}
									?>
								</select>
							</div>
